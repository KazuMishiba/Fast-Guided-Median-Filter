...
*/
	...
	*/
	template<>
	float WMF::calculateHcum<int, float>(const int& f_cum, const int& g_cum, const float& c_x, const float &d_x)
	{
		return c_x * g_cum + d_x * f_cum;
	}
	template<>
	float WMF::calculateHcum<cv::Vec3i, cv::Vec3f>(const int& f_cum, const cv::Vec3i& g_cum, const cv::Vec3f& c_x, const float& d_x)
	{
