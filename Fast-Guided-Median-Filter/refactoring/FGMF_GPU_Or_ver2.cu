...
namespace FGMF_GPU_Or_ver2
{
	...
	* このとき、ウィンドウ分のデータをとりあえず各スレッドが勝手に読むことにするが、これを各スレッドに並列に行わせてshared memoryに入れる。
	* ⇒median tracking無しで実装したが、ものすごく遅くなったのでダメ。多分トラッキング入れてもそこまで速くならないだろう。
	*/
	...
	*/
	...
	__device__ inline void
		de_readPixel(cudaTextureObject_t* texF, cudaTextureObject_t* texG, int s, int t, int2& readData)
	{
		readData.x = tex2D<int>(*texF, s, t);
		readData.y = tex2D<int>(*texG, s, t);
	}


	__device__ inline void
		de_addPixelToWindow(int2* W_FG, int2* readData, int tid, int pixelNum)
	{
		for (int i = 0; i < pixelNum; i++)
		{
			...
			{
				W_FG[tid].x += 1;
				W_FG[tid].y += readData[i].y;
			}		
		}
	}

	__device__ inline void
		de_removePixelFromWindow(int2* W_FG, int2* readData, int tid, int pixelNum)
	{
		for (int i = 0; i < pixelNum; i++)
		{
			if (readData[i].x == tid)
		...
		de_filter2dF1(int width, int height, int radius, int fRange, int* result_center, cudaTextureObject_t* texF, cudaTextureObject_t* texG, DC_TYPE* dc, size_t pitchI1, size_t pitchF_channelNum_fg)
	{
		...
		int tid = threadIdx.x;
		if (s_center >= width || tid >= fRange)
			return;
		int s_relative = tid - radius;//処理中心列からの距離
		int s_handle = s_center + s_relative;//処理する画素


		//FG_TYPE* W_fg_cum = &sharedData<FG_TYPE>[0];
		//FG_TYPE* W_FG = &sharedData<FG_TYPE>[channelNum_fg];
		FG_TYPE* W_FG = &sharedData<FG_TYPE>[0];
		FG_TYPE* readData = &sharedData<FG_TYPE>[fRange];//各スレッドが読み込んだ画素　量は2r+1
		//__shared__ int W_k;

		if (tid == 0)
		{
			//ヒストグラム初期化, fgSumUpToIndex初期化
			memset(sharedData<FG_TYPE>, 0, sizeof(FG_TYPE) * fRange + sizeof(FG_TYPE) * (2 * radius + 1));
			//W_k = tex2D<int>(*texF, s_center, 0);//current index
		}

		__syncthreads();
		//1つ目ヒストグラム形成
		//まず各スレッドが半径内のx方向のヒストグラム形成は各スレッドが担当する
			// -r から r までを担当するスレッドが処理
		for (int t = -radius; t <= radius; t++)
		{
			//画素行の読み込み
			if (s_relative <= radius)
			{
				de_readPixel(texF, texG, s_handle, t, readData[tid]);
			}
			__syncthreads();
			de_addPixelToWindow(W_FG, readData, tid, 2*radius+1);
		}
		...
		if (tid == 0)
		{
			...
			FG_TYPE W_fg_cum;
			memset(&W_fg_cum, 0, sizeof(FG_TYPE));
			int W_k = -1;
		...
		for (int t = 1; t < height; t++)
		{
			...
			if (s_relative <= radius)
			{
				de_readPixel(texF, texG, s_handle, t + radius, readData[tid]);
			}
			__syncthreads();
			//ヒストグラムに追加
			de_addPixelToWindow(W_FG, readData, tid, 2 * radius + 1);
			...
			if (s_relative <= radius)
			{
				de_readPixel(texF, texG, s_handle, t - radius - 1, readData[tid]);
			}
			__syncthreads();
			//ヒストグラムから削除
			de_removePixelFromWindow(W_FG, readData, tid, 2 * radius + 1);
...
/*
	...
	*/
	...
	void cu_filter2DF1(Helper::SizeInfo& sizeInfo, cudaStream_t stream, int radius, int fRange, Helper::DeviceArray<int>*& result_center, Helper::DeviceArray<int>* f, Helper::DeviceArray<int>* g, DC_TYPE* dc)
	{
		...
		//sizeof(FG_TYPE) * fRange + sizeof(FG_TYPE) * (2 * radius + 1)

		de_filter2dF1<FG_TYPE, DC_TYPE> << <gridSizeX, blockSize, sizeof(FG_TYPE)* fRange + sizeof(FG_TYPE) * (2 * radius + 1), stream >> > (sizeInfo.width_, sizeInfo.height_, radius, fRange, result_center->host[0], texF.device, texG.device, dc, sizeInfo.pitch<int>(), sizeInfo.pitch<DC_TYPE>());
	...
	*/
	...
	void cu_filter2DF1(Helper::SizeInfo& sizeInfo, cudaStream_t stream, int radius, int fRange, Helper::DeviceArray<int>*& result_center, Helper::DeviceArray<int>* f, Helper::DeviceArray<int>* g, float2* dc)
	{
		...
		de_filter2d<int2, float2> << <gridSizeX, blockSize, (fRange + 1) * sizeof(int)* k, stream >> > (sizeInfo.width_, sizeInfo.height_, radius, fRange, result_center->host[0], texF.device, texG.device, dc, sizeInfo.pitch<int>(), sizeInfo.pitch<float2>());
	...
	void cu_filter2DF1(Helper::SizeInfo& sizeInfo, cudaStream_t stream, int radius, int fRange, Helper::DeviceArray<int>*& result_center, Helper::DeviceArray<int>* f, Helper::DeviceArray<int>* g, float4* dc)
	{
		...
		de_filter2d<int4, float4> << <gridSizeX, blockSize, (fRange + 1) * sizeof(int)* k, stream >> > (sizeInfo.width_, sizeInfo.height_, radius, fRange, result_center->host[0], texF.device, texG.device, dc, sizeInfo.pitch<int>(), sizeInfo.pitch<float4>());
	}
	//I1GX
