...
/*
（３）１行更新方式 (O(r)-sliding window)
GPU実装 GPU-O(r)
メインウィンドウのみで、ヒストグラムとインデックス以下和をもつタイプ。メリットは、ウィンドウサイズが小さいときにおそらくもっとも高速。列ウィンドウに相当する画素数の追加と削除だが、ヒストグラムの追加・削除要素のみにアクセスすればよいので、256回演算が不要。列に含まれる画素数*2(追加と削除)＋α（インデックス移動分）かかる。ただしベクトル演算ができない。一方でGPU実装で高速化の可能性。

GPU実装2D3D完了

CPU実装開始2Dのみ予定（やめるGPUのみにする）
*/
